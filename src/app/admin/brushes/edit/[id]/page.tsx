...
import { getBrushById, saveBrush, getCategories } from '@/lib/supabase-client';

export default function EditBrushPage() {
  ...
  useEffect(() => {
    const fetchBrush = async () => {
      try {
        ...
        // 调用Supabase API获取笔刷数据
        const brush = await getBrushById(brushId);
        
        if (!brush) {
          throw new Error('未找到笔刷');
        }
        
        setFormData({
          title: brush.title,
          description: brush.description,
          longDescription: brush.long_description || '',
          category: brush.category_id,
          price: brush.price?.toString() || '0',
          isFree: brush.is_free,
          isFeatured: brush.is_featured,
          compatibility: brush.compatibility || 'Procreate 5+',
          tags: Array.isArray(brush.tags) ? brush.tags.join(', ') : (brush.tags || ''),
          file: null,
          previewImage: null,
          galleryImages: [],
          existingGalleryImages: brush.brush_images?.map((img: { image_url: string }) => img.image_url) || [],
          downloadUrl: brush.file_url || '',
          fileSize: brush.file_size || ''
        });
        
        setPreviewUrl(brush.preview_image_url);
        
      } catch (err) {
        setError(err instanceof Error ? err.message : '加载笔刷数据失败');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const brushDataToSave = {
        ...
        galleryImagesToDelete: [], // 实际使用中，这里应该从UI收集要删除的图片ID
      };
      ...
      setError(err instanceof Error ? err.message : '更新笔刷失败，请重试');
      window.scrollTo(0, 0);
    } finally {
      setIsSaving(false);
    }
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">编辑笔刷</h1>
        <Link
          href="/admin"
          ...
          <div className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                预览图 {formData.previewImage === null && !previewUrl && '*'}
              </label>
            ...
            <div>
              ...
              {formData.existingGalleryImages.length > 0 ? (
                <div className="mt-2 grid grid-cols-3 gap-2">
                  {formData.existingGalleryImages.map((imageUrl, index) => (
                    <div key={index} className="relative h-24 bg-gray-100 dark:bg-gray-700 rounded">
